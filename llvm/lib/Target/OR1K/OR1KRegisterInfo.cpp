...
OR1KRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  if ((FrameIndex >= MinCSFI && FrameIndex <= MaxCSFI) &&
    ...
    if (!HasFP || (shouldRealignStack(MF) && FrameIndex >= 0))
  ...
  if (!isInt<16>(Offset)) {
    ...
    if (!Reg)
       Reg = RS->scavengeRegisterBackwards(OR1K::GPRRegClass, II, false /* TODO check */, SPAdj);
...
bool OR1KRegisterInfo::hasBasePointer(const MachineFunction &MF) const {
   ...
   if (shouldRealignStack(MF) && MFI.hasVarSizedObjects())
     return true;
